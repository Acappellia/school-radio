...
execute at @a as @e[distance=16..64,sort=random,limit=1,type=item_display,tag=lootable,tag=!untouched] if score @s chest_loottime < #chest_refresh_time chest_loottime at @s run function sr:refresh/chest_reset_self